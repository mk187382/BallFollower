...
float ballDiameter = 7; //centimeters
int focal = 2560; //px
cv::Mat imgOriginal;
...
std::pair <std::vector<std::vector<cv::Point>>, std::vector<cv::Vec3i>> getContoursAndCircles(cv::Mat imageToSearchContoursIn)
{
	...
	for (auto contour : contouredElementsOnScreen)
	{
		...
		double radius = std::sqrt(area / math_PI);
...
float getBallDistance(cv::Point blob, cv::Point screenCenter, float ballDiam, int radiusPx) {

	float angle;

	float distance = focal * ballDiameter*0.1 / (radiusPx * 2);//cm
	cv::Point startCountAngleHere(screenCenter.x, imgOriginal.rows);
	float resolution = (2*radiusPx)/ballDiameter; //[px/cm]
	int distFromCenterPx = blob.x - screenCenter.x; //[px]
	float distFromCenterCentimeters = distFromCenterPx / resolution;//[cm]
	if (distFromCenterPx > 0 )
		angle = asin(distFromCenterCentimeters / distance) * (180 / math_PI);//deg
	else 
		angle = -1*asin(abs(distFromCenterCentimeters) / distance) * (180 / math_PI);//deg

	std::cout << "\ndistance = " << distance << "\nangle = " << angle << "\nfrom center: " << distFromCenterCentimeters << std::endl;
	cv::line(imgOriginal, startCountAngleHere, blob, cv::Scalar(0, 200, 255), 5); //yellow line
	return angle;
}
...
int main()
{
	...
	while (true)
	{
		...
		float angle = getBallDistance(blobCenter, cv::Point (imgOriginal.cols / 2, imgOriginal.rows / 2), ballDiameter, radius);
